...
def build_edit_prompt(page: dict, review_notes: dict | None = None) -> str:
    ...
    if review_notes:
        ...
        if preserve_dimensions:
            sections.append(prompt_items("PASSED QUALITY DIMENSIONS — PRESERVE", preserve_dimensions))
        if failed_dimensions:
            sections.append(prompt_items("FAILED QUALITY DIMENSIONS — CORRECT", failed_dimensions))
        if text:
            sections.append(f"HUMAN CORRECTION REQUEST: {text}")
        if tags:
            sections.append(prompt_items("HUMAN QUICK CHANGES", tags))
            sections.append(prompt_items("TARGETED REMEDIATION", expand_defect_tags(ROOT, tags)))
